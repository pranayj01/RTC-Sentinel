...
const JWT_ISSUER = 'rtc-sentinel';
const JWT_AUDIENCE = 'rtc-sentinel-api';
...
function expiryFor(kind: TokenKind): SignOptions['expiresIn'] {
  ...
  return (value ??
    (kind === 'access' ? '15m' : '7d')) as SignOptions['expiresIn'];
}
function sign(userId: string, kind: TokenKind): string {
  return jwt.sign({ type: kind }, secretFor(kind), {
    algorithm: 'HS256',
    issuer: JWT_ISSUER,
    audience: JWT_AUDIENCE,
    subject: userId,
    expiresIn: expiryFor(kind),
  });
}
function verify(token: string, kind: TokenKind): string {
  ...
  if (payload.type !== kind || typeof payload.sub !== 'string')
