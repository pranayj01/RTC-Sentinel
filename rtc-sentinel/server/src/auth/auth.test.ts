...
describe('authentication API', () => {
  ...
  it('valid registration', async () => {
    const response = await request(createApp(users))
      .post('/auth/register')
      .send(valid);
    expect(response.status).toBe(201);
    expect(response.body.user.email).toBe(valid.email);
    expect(response.body.user.passwordHash).toBeUndefined();
    expect(users.users[0].passwordHash).not.toBe(valid.password);
  });
  ...
  it('invalid email', async () => {
    const response = await request(createApp(users))
      .post('/auth/register')
      .send({ ...valid, email: 'bad' });
    expect(response.status).toBe(400);
  });
  it('weak password', async () => {
    const response = await request(createApp(users))
      .post('/auth/register')
      .send({ ...valid, password: 'password' });
    expect(response.status).toBe(400);
  });
  ...
  it('wrong password', async () => {
    ...
    const response = await request(app)
      .post('/auth/login')
      .send({ email: valid.email, password: 'WrongPass1' });
    expect(response.status).toBe(401);
  });
  it('expired JWT', async () => {
    const token = jwt.sign({ type: 'access' }, 'test-access-secret', {
      algorithm: 'HS256',
      issuer: 'rtc-sentinel',
      audience: 'rtc-sentinel-api',
      subject: 'user-1',
      expiresIn: -1,
    });
    const response = await request(createApp(users))
  ...
  it('rejects JWTs signed with an unapproved algorithm', async () => {
    const token = jwt.sign({ type: 'access' }, 'test-access-secret', {
      algorithm: 'HS512',
      issuer: 'rtc-sentinel',
      audience: 'rtc-sentinel-api',
  ...
  it('protected API without JWT', async () => {
    const response = await request(createApp(users)).get('/users/me');
    expect(response.status).toBe(401);
  });
  it('protected route and token refresh work', async () => {
    const app = createApp(users);
    const agent = request.agent(app);
    const registered = await agent.post('/auth/register').send(valid);
    const me = await request(app)
      .get('/users/me')
      .set('Authorization', `Bearer ${registered.body.accessToken}`);
    expect(me.status).toBe(200);
    expect(me.body.user.email).toBe(valid.email);
    const refreshed = await agent.post('/auth/refresh');
    expect(refreshed.status).toBe(200);
    expect(refreshed.body.accessToken).toEqual(expect.any(String));
    expect(refreshed.body.refreshToken).toBeUndefined();
  });
  it('rejects missing refresh cookies and clears them on logout', async () => {
    const app = createApp(users);
    const missing = await request(app).post('/auth/refresh');
    expect(missing.status).toBe(401);
    expect(missing.body.error.code).toBe('INVALID_TOKEN');
    const malformed = await request(app)
      .post('/auth/refresh')
      .set('Cookie', 'rtc_sentinel_refresh=not-a-jwt');
